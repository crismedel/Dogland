...
const MySightingCard = ({ sighting, onPress }: { sighting: MySighting, onPress: () => void }) => {
    ...
    const estadoAvistamientoNombre = obtenerNombreEstadoSalud(sighting.id_estado_avistamiento); // Usando un solo helper temporal
    const isCritical = sighting.id_estado_salud === 3;
    const shortDescription = sighting.descripcion.length > 50 ? sighting.descripcion.substring(0, 50) + '...' : sighting.descripcion;

    const formatDate = (dateString: string) => {
        try { return new Date(dateString).toLocaleDateString(); } catch { return 'Fecha inválida'; }
    };
    ...
    return (
        ...
        >
            ...
            <View style={styles.infoRow}>
                <Ionicons name="paw-outline" size={16} color={Colors.darkGray} style={{marginRight: 8}} />
                <AppText style={styles.label}>Especie ID:</AppText>
                <AppText style={styles.value}>{sighting.id_especie}</AppText> 
            </View>
            
            <View style={styles.infoRow}>
                <Ionicons name="medkit-outline" size={16} color={isCritical ? Colors.danger : Colors.darkGray} style={{marginRight: 8}} />
            ...
            <View style={styles.infoRow}>
                <Ionicons name="list-outline" size={16} color={Colors.darkGray} style={{marginRight: 8}} />
                <AppText style={styles.label}>Estado Av. ID:</AppText>
                <AppText style={styles.value}>{sighting.id_estado_avistamiento}</AppText>
            </View>

            <AppText style={styles.cardDate}>Reportado: {formatDate(sighting.fecha_creacion)}</AppText>
        </TouchableOpacity>
    );
};
...
const MySightingsScreen = () => {
    ...
    const fetchMySightings = useCallback(async (showNotification = false) => {
    ...
    }, [loading, refreshing, showSuccess, initialLoadComplete]); // Añadimos initialLoadComplete a las dependencias
    ...
    if (loading && sightings.length === 0 && !error) {
        return (
            <View style={styles.centered}>
                ...
                <AppText style={styles.loadingText}>Cargando tus avistamientos...</AppText>
            </View>
        );
    }
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            {error ? (
                ...
                <View style={styles.centered}>
                    <AppText style={styles.emptyText}>Aún no has creado ningún avistamiento. ¡Anímate a reportar!</AppText>
                    <TouchableOpacity onPress={() => router.push('/create-report')} style={styles.retryButton}>
...
const styles = StyleSheet.create({
  ...
  errorText: { color: Colors.danger, textAlign: 'center', marginBottom: 15, fontSize: 16, fontWeight: '700' },
  retryButton: {
