...
export const updateSighting = async (req, res) => {
    try {
        ...
        if (!oldSighting) {
            return res.status(404).json({ success: false, message: 'Avistamiento no encontrado para auditar' });
        }
    ...
    try {
        ...
        if (error.code === '23503') {
             return res.status(500).json({
                success: false,
                error: 'No se puede eliminar el avistamiento porque está siendo referenciado por otros registros.',
            });
        }
...
export const filterSightings = async (req, res) => {
    ...
    try {
        ...
        if (id_estado_salud && id_estado_salud !== '' && id_estado_salud !== 'null') {
            filters.push(`av.id_estado_salud = $${paramIndex}`);
            values.push(parseInt(id_estado_salud));
            paramIndex++;
        }

        if (id_estado_avistamiento && id_estado_avistamiento !== '' && id_estado_avistamiento !== 'null') {
