...
export const validateAlert = async (req, res, next) => {
  ...
  if (isCreate) {
    ...
    if (!titulo || !descripcion || !id_tipo_alerta || !id_nivel_riesgo) {
      return res
        .status(400)
        .json({ success: false, error: 'Campos requeridos faltantes' });
    }
  } else {
    // En PUT al menos un campo debe estar presente
    if (!titulo && !descripcion && !id_tipo_alerta && !id_nivel_riesgo) {
      return res
        .status(400)
        .json({ success: false, error: 'Debe proporcionar al menos un campo para actualizar' });
    }
  }
 ...
 if (titulo !== undefined) {
  ...
  if (id_tipo_alerta !== undefined && isNaN(parseInt(id_tipo_alerta, 10))) {
    return res
      .status(400)
