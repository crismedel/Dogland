...
const EditSightingScreen = () => {
    ...
    useEffect(() => {
        const fetchOptions = async () => {
            try {
                ...
                const estadosSaludFormateados = healthRes.data.data.map((item: Option) => ({
                    label: obtenerNombreEstadoSalud(item.id), // Usar la función de report.ts
                    value: item.id
                }));
                
                // Para estados de avistamiento - usar el nombre de la API
                const estadosAvistamientoFormateados = statusRes.data.data.map((item: Option) => ({
    ...
    useEffect(() => {
        const fetchSighting = async () => {
            ...
            try {
                ...
                const currentLocation = (sighting.latitude && sighting.longitude) 
    ...
    const hasChanges = () => {
        ...
        const ubicacionChanged = JSON.stringify(formData.ubicacion) !== JSON.stringify(originalData.ubicacion);
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.id_especie || !formData.id_estado_salud || !formData.id_estado_avistamiento) {
        ...
        try {
            ...
            if (err.response?.status === 403) {
                ...
                Alert.alert('Error del Servidor', 'No se pudo guardar la actualización.');
            }
        } finally {
            ...
            >
                ...
                >
                    <View style={styles.formContainer}>
                        ...
                        <TextInput
                            style={[styles.textInput, styles.inputMultiline, !canEdit && styles.disabledInput]}
                        ...
                        <View style={[styles.inputContainer, Platform.OS === 'ios' && { zIndex: 3000 }]}>
                            ...
                            <DropDownPicker
                                ...
                                setValue={(callback) => {
                                    ...
                                    setFormData(prev => ({ ...prev, id_estado_avistamiento: value }));
                                }}
                        ...
                        {/* --- ESTADO DE SALUD --- */}
                        <View style={[styles.inputContainer, Platform.OS === 'ios' && { zIndex: 1000 }]}>
